...
# Appends the grep-for-windows functions to the current user's PowerShell profile.
# Creates the profile file (and its parent directory) if they do not exist.
...
# Extract the version embedded in $grepCode.
$newVersion = if ($grepCode -match '# version:\s*(.+)') { $Matches[1].Trim() } else { $null }
...
# Check whether grep-for-windows is already present in the profile.
if ($profileContent -match '# --- grep-for-windows: start ---') {
    ...
    if ($installedVersion -eq $newVersion) {
        Write-Host "grep-for-windows $installedVersion is already installed at: $profilePath. No changes made."
    } else {
        ...
        # Reload the profile so the updated grep is available in the current session.
        . $profilePath
        Write-Host "Profile reloaded. 'grep' is ready to use."
    }
} else {
    Add-Content -Path $profilePath -Value $grepCode -Encoding UTF8
    Write-Host "grep-for-windows $newVersion added to: $profilePath"

    # Reload the profile so grep is available in the current session.
    . $profilePath
    Write-Host "Profile reloaded. 'grep' is ready to use."
}
